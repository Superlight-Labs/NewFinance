...
void circuit_def_t::copy_bits(byte_ptr dst, int dst_index, const_byte_ptr src, int src_index, int count)
{
  for (int i = 0; i < count; i++)
    set_bit(dst, dst_index + i, get_bit(src, src_index + i));
}
...
void circuit_def_t::resolve_const_wires(wires_t &wires)
{
  ...
  {
    if (wires[i] == gc_true)
      wires[i] = get_wire_true();
    else if (wires[i] == gc_false)
      wires[i] = get_wire_false();
  }
}
...
{
  int n_in1 = ub::le_get_4(bin);
  bin += 4;
  int n_in2 = ub::le_get_4(bin);
  bin += 4;
  int n_out = ub::le_get_4(bin);
  bin += 4;
  ...
  for (;;)
  {
    ...
    {
    case gate_type_not:
      in1 = get_compressed(bin, in1_size) + old_in1 + 1;
      old_in1 = in1;
      update_input_wire(in1, input1, input2, offset);
      out = get_compressed(bin, out_size) + old_out + 1;
      old_out = out;
    ...
    case gate_type_and:
      ...
      old_in1 = in1;
      update_input_wire(in1, input1, input2, offset);
      in2 = get_compressed(bin, in2_size) + old_in2 + 1;
      old_in2 = in2;
  ...
  for (int i = 0; i < n_out; i++)
    output[i] = n_wires - n_out + i;
}

int circuit_def_t::or_gate(int res, int &w1, int &w2)
{
  if (w1 == gc_true || w2 == gc_true)
    return gc_true;
  if (w1 == gc_false)
    return w2;
  if (w2 == gc_false)
...
{
  return oword_simple_shift_left(g1, 1) ^
         tweak;
}
...
template <class T>
void gc_garble(T &fixed_key, int n_gates, const circuit_def_t::gate_t *gates, buf128_t *garbled, buf128_t *wires, buf128_t delta)
{
...
template <class T>
void gc_evaluate(T &fixed_key, int n_gates, const circuit_def_t::gate_t *gates, const buf128_t *garbled, buf128_t *wires)
{
  for (int index = 0; index < n_gates; index++)
  {
    garbled_circuit_t::wire_t input0 = wires[gates[index].input0];
