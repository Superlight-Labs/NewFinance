...
namespace crypto
{
  ...
  static EC_GROUP *get_optimized_curve(int type)
  {
    ...
    if (group)
      EC_GROUP_precompute_mult(group, NULL);
    if (!group)
      ERR_clear_error();
    return group;
  }
  ...
  enum
  {
    BCRYPT_ECDH_P256_ALGORITHM = 0,
    BCRYPT_ECDSA_P256_ALGORITHM = 0,
    BCRYPT_ECDH_PUBLIC_P256_MAGIC = 0,
    BCRYPT_ECDSA_PUBLIC_P256_MAGIC = 0,
    BCRYPT_ECDH_PRIVATE_P256_MAGIC = 0,
    BCRYPT_ECDSA_PRIVATE_P256_MAGIC = 0
  };
  enum
  {
    BCRYPT_ECDH_P384_ALGORITHM = 0,
    BCRYPT_ECDSA_P384_ALGORITHM = 0,
    BCRYPT_ECDH_PUBLIC_P384_MAGIC = 0,
    BCRYPT_ECDSA_PUBLIC_P384_MAGIC = 0,
    BCRYPT_ECDH_PRIVATE_P384_MAGIC = 0,
    BCRYPT_ECDSA_PRIVATE_P384_MAGIC = 0
  };
  enum
  {
    BCRYPT_ECDH_P521_ALGORITHM = 0,
    BCRYPT_ECDSA_P521_ALGORITHM = 0,
    BCRYPT_ECDH_PUBLIC_P521_MAGIC = 0,
    BCRYPT_ECDSA_PUBLIC_P521_MAGIC = 0,
    BCRYPT_ECDH_PRIVATE_P521_MAGIC = 0,
    BCRYPT_ECDSA_PRIVATE_P521_MAGIC = 0
  };
#endif
  ...
  ecurve_t ecurve_t::find(const_char_ptr name)
  {
    for (int i = 0; i < _countof(g_curves); i++)
    {
      ...
      if (!curve.get_group())
        continue;
      if (0 == strcmp(curve.get_name(), name))
        return curve;
    }
  ...
  ecurve_t ecurve_t::find(int openssl_id)
  {
    ...
    for (int i = 0; i < _countof(g_curves); i++)
    {
      ...
      if (!curve.get_group())
        continue;
      if (openssl_id == curve.get_openssl_code())
        return curve;
    }
  ...
  ecurve_t ecurve_t::find_dyadic(int dyadic_id)
  {
    for (int i = 0; i < _countof(g_curves); i++)
    {
      ...
      if (!curve.get_group())
        continue;
      if (dyadic_id == curve.get_dyadic_code())
        return curve;
    }
  ...
  ecurve_t ecurve_t::find_kmip(int kmip_id)
  {
    for (int i = 0; i < _countof(g_curves); i++)
    {
      ...
      if (!curve.get_group())
        continue;
      if (kmip_id == curve.get_kmip_code())
        return curve;
    }
  ...
  ecurve_t ecurve_t::find(const EC_GROUP *group)
  {
    ...
    for (int i = 0; i < _countof(g_curves); i++)
    {
      ...
      if (eq_points(group, EC_GROUP_get0_generator(group), curve_group, EC_GROUP_get0_generator(curve_group)))
        return curve;
    }
  ...
  int ecurve_t::point_to_oct(const EC_POINT *point, byte_ptr out) const
  {
    int n = get_oct_point_size();
    if (out)
      EC_POINT_point2oct(get_group(), point, POINT_CONVERSION_UNCOMPRESSED, out, n, bn_t::tls_bn_ctx());
    return n;
  }
  ...
  int ecurve_t::point_to_compressed_oct(const EC_POINT *point, byte_ptr out) const
  {
    ...
    if (out)
      EC_POINT_point2oct(get_group(), point, POINT_CONVERSION_COMPRESSED, out, n, bn_t::tls_bn_ctx());
    return n;
  }
  ...
  ecc_point_t ecurve_t::der_to_point(mem_t in) const
  {
    ...
    if (octet_string)
      return oct_to_point(mem_t(octet_string, get_oct_point_size()));
  ...
  void ecc_point_t::convert(ub::converter_t &converter)
  {
    ...
    if (converter.is_write())
    {
      if (!converter.is_calc_size())
        to_compressed_oct(converter.current());
    }
    else
    {
      if (value_size < 0)
  ...
  {
    if (0 < EC_POINT_set_compressed_coordinates_GFp(curve.get_group(), *this, x, y_bits, bn_t::tls_bn_ctx()))
      return true;
...
#define OPENSSL_ECDSA_SIG_PTR
  ...
  int ecdsa_signature_t::to_bin(byte_ptr out) const
  {
    ...
    if (r_size > curve_size || s_size > curve_size)
      return -1;
  ...
  {
    if (recovery_code != 0 && recovery_code != 1)
      return ecc_point_t();
  ...
  {
    if (out)
      ec25519_core::ge_p3_tobytes(out, &ge_p3);
    return 32;
  }
