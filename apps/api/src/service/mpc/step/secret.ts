...
export const processGenericSecret = async (
  ...
  try {
    const wallet = await saveKeyShare(user, context.getNewShare().toString('base64'), 'secret');

    logger.info(
      {
        ...wallet,
        genericSecret: wallet.value?.slice(0, 23),
      },
      'Generic Secret Created'
    );

    connection.socket.send(JSON.stringify({ done: true, serverShareId: wallet.id }));
    ...
    connection.socket.close(undefined, 'Error while saving generic Secret from generate');
  }
};
